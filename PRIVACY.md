...
For maximum privacy, use the local npm package:
    ...
    npx @ansvar/french-standards-mcp
...
All data in this MCP is sourced from publicly available French government
publications from ANSSI (ssi.gouv.fr), CNIL (cnil.fr), and ANS (esante.gouv.fr).
See `sources.yml` for the full list of data origins and their licenses.
